...
Public Class DisplayItemsNew
    ...
    Sub DisplayItem(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        Dim MyKeysArr As String() = Nothing
        ...
        If AllowAccess Then
            If Sql <> "" Then
                ...
                End If
            Else
                MyKeysArr = MyKeys.Split(New String() {"~~~"}, StringSplitOptions.RemoveEmptyEntries)
                Select Case mySearchTable
                    Case "Warehouse_PO"
                        Dim Facility As String = MyKeysArr(0)
                        Dim POKey As String = MyKeysArr(1)
                    ...
                    Case "Warehouse_ASN"
                        Dim Facility As String = MyKeysArr(0)
                        Dim ReceiptKey As String = MyKeysArr(1)
                        Dim MyWarehouse As String = CommonMethods.getFacilityDBName(Facility)

                        Dim Xml As String = "<Message><Head><MessageID>0000000003</MessageID><MessageType>AdvancedShipNotice</MessageType><Action>list</Action><Sender>	<User>" & CommonMethods.username & "</User>			<Password>" & CommonMethods.password & "</Password><SystemID>MOVEX</SystemID>	<TenantId>INFOR</TenantId>	</Sender><Recipient><SystemID>" & MyWarehouse & "</SystemID></Recipient></Head><Body><AdvancedShipNotice><AdvancedShipNoticeHeader>  <ReceiptKey>" & ReceiptKey & "</ReceiptKey> </AdvancedShipNoticeHeader>		</AdvancedShipNotice></Body></Message>"
                        Dim ViewTable As DataTable = CommonMethods.ViewXml(Xml)
                        MyError = ViewTable.Rows(0)!error
                        Dim MyDoc As XmlDocument = ViewTable.Rows(0)!doc
                        If MyError = "" Then
                            ...
                            For Each node As XmlNode In results
                                ...
                                If Not node("Status").IsEmpty Then
                                    Dim DTReceiptStatus = CommonMethods.getCodeDD(MyWarehouse, "codelkup", "RECSTATUS")
                                ...
                                If Not node("Type").IsEmpty Then
                                    Dim DTReceiptType = CommonMethods.getCodeDD(MyWarehouse, "codelkup", "RECEIPTYPE")
                    ...
                    Case "Warehouse_SO"
                        ...
                        If MyError = "" Then
                            ...
                            For Each node As XmlNode In results
                                ...
                                If Not node("Status").IsEmpty Then
                                    Dim DTOrderStatus = CommonMethods.getCodeDD(MyWarehouse, "orderstatussetup", "ORDERSTATUS")
                                ...
                                If Not node("Type").IsEmpty Then
                                    Dim DTOrderType = CommonMethods.getCodeDD(MyWarehouse, "codelkup", "ORDERTYPE")
            ...
            If mySearchTable.Contains("ORDERMANAG") Then
                ...
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    Next
                End If
            ElseIf mySearchTable = "Warehouse_PO" Or mySearchTable = "Warehouse_ASN" Or mySearchTable = "Warehouse_SO" Then
                Warehouse = CommonMethods.getFacilityDBName(MyKeysArr(0))
                If Warehouse <> "" Then
                    If LCase(Warehouse.Substring(0, 6)) = "infor_" Then Warehouse = Warehouse.Substring(6, Warehouse.Length - 6)
                    If LCase(Warehouse).Contains("_") Then Warehouse = Warehouse.Split("_")(1)
                End If
                KeyValue = MyKeysArr(1)
            ...
            End If
        Else
            MyError = "You are not allowed to view this record"
        End If
    ...
    End Sub
    Private Function AllowItemAccess(ByVal MyID As String) As Boolean
        Dim AllowAccess As Boolean = True
        Dim AndFilter As String = ""
        ...
        If owners IsNot Nothing Then
            ...
            AndFilter += " and SerialKey = '" & MyID & "'"

            Dim sql As String = "select * from enterprise.sku where 1=1 " & AndFilter
            Dim ds As DataSet = (New SQLExec).Cursor(sql)
            AllowAccess = ds.Tables(0).Rows.Count > 0
        End If

        Return AllowAccess
    End Function
    Private Function AllowItemCatalogueAccess(ByVal MyID As String) As Boolean
        Dim AllowAccess As Boolean = True
        Dim AndFilter As String = ""
        Dim owners As String() = CommonMethods.getOwnerPerUser(HttpContext.Current.Session("userkey").ToString)
        ...
        If owners IsNot Nothing Then
            ...
            Dim sql As String = "select * from " & IIf(CommonMethods.dbtype <> "sql", "SYSTEM.", "") & "SKUCATALOGUE where 1=1 " & AndFilter
            Dim ds As DataSet = (New SQLExec).Cursor(sql)
            AllowAccess = ds.Tables(0).Rows.Count > 0
        End If
    ...
    Private Function AllowPurchaseOrderAccess(ByVal MyKeys As String) As Boolean
        ...
        If dr.Count > 0 Then
            ...
            If owners IsNot Nothing And suppliers IsNot Nothing Then
                Dim AndFilter As String = " and POKey ='" & POKey & "'"
                ...
                Dim sql As String = "Select * from " & warehouselevel & ".PO where 1=1 " & AndFilter
                Dim ds As DataSet = (New SQLExec).Cursor(sql)
                AllowAccess = ds.Tables(0).Rows.Count > 0
            End If
        Else
        ...
        Return AllowAccess
    End Function
    Private Function AllowASNAccess(ByVal MyKeys As String) As Boolean
        Dim MyKeysArr As String() = MyKeys.Split(New String() {"~~~"}, StringSplitOptions.RemoveEmptyEntries)
        Dim Facility As String = MyKeysArr(0)
        Dim ReceiptKey As String = MyKeysArr(1)
        Dim AllowAccess = True
        ...
        If dr.Count > 0 Then
            Dim owners As String() = CommonMethods.getOwnerPerUser(HttpContext.Current.Session("userkey").ToString)
            If owners IsNot Nothing Then
                Dim AndFilter As String = " and ReceiptKey ='" & ReceiptKey & "'"
                ...
                Dim sql As String = "Select * from " & warehouselevel & ".RECEIPT where 1=1 " & AndFilter
                Dim ds As DataSet = (New SQLExec).Cursor(sql)
                AllowAccess = ds.Tables(0).Rows.Count > 0
            End If
        Else
        ...
        Return AllowAccess
    End Function
    Private Function AllowSOAccess(ByVal MyKeys As String) As Boolean
        Dim MyKeysArr As String() = MyKeys.Split(New String() {"~~~"}, StringSplitOptions.RemoveEmptyEntries)
        Dim Facility As String = MyKeysArr(0)
        Dim OrderKey As String = MyKeysArr(1)
        Dim AllowAccess = True
        ...
        If dr.Count > 0 Then
            ...
            If owners IsNot Nothing And consignees IsNot Nothing Then
                Dim AndFilter As String = " and OrderKey ='" & OrderKey & "'"
                ...
                Dim sql As String = "Select * from " & warehouselevel & ".ORDERS where 1=1 " & AndFilter
                Dim ds As DataSet = (New SQLExec).Cursor(sql)
                AllowAccess = ds.Tables(0).Rows.Count > 0
            End If
        Else
        ...
        Return AllowAccess
    End Function
    Private Function AllowOrderManagementAccess(ByVal MyID As String) As Boolean
        Dim AllowAccess As Boolean = True
        ...
        If i > 0 Then
            ...
            If owners IsNot Nothing And consignees IsNot Nothing Then
                ...
                Dim sql As String = "select * from " & IIf(CommonMethods.dbtype <> "sql", "SYSTEM.", "") & "ORDERMANAG where 1=1 " & AndFilter
                Dim ds As DataSet = (New SQLExec).Cursor(sql)
                AllowAccess = ds.Tables(0).Rows.Count > 0
            End If
        Else
